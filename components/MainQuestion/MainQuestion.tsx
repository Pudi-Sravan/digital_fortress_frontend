  ...
  const checkAnswer = async (): Promise<boolean> => {
    try {
      const checkResponse = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}quiz/checkRound`,
        {
          answer: answer,
        },
        {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={style.bottomSection}>
            <div className={style.inputSection}>
              ...
              />
              {isCorrect === -1 && (
                <p className={style.feedback}>Incorrect Answer</p>
              )}
            </div>
            {/* {deviceType === "monitor" && ( */}
            <div className={style.row}>
              ...
              <div
                onClick={() => setIsClueModalActive(true)}
                className={style.clueButton}
              >
                CLUES
              </div>
            </div>
